...
"При запросах погоды, учитывай текущую дату и время, чтобы понять про какой именно день спрашивает пользователь. Данные погоды бери только из поиска с сайта Яндекс.Погоды в соответствии с нужной датой и местом, и никогда не выдумывай. Если в поиске данных Яндекс.Погоды нет, то отвечай, что не имеешь возможности подключиться к надежным серверам погоды."
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_message_plain_parts = [
        ...
        f"- умею читать и понимать изображения и документы, а также контекст веб-страниц по ссылкам.",
