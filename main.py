...
# - Убран импорт GenerationResponse.
# - Аннотация типа для параметра response в _parse_gemini_response изменена на Any.
# - Добавлены проверки hasattr в _parse_gemini_response для большей надежности.
# - Обработка документов в handle_document все еще требует рефакторинга для исправления AttributeError.
...
import datetime
import pytz
from typing import Any # Для аннотации типа Any
...
# from google.generativeai.types import GenerationResponse # УДАЛЕНО: Вызывало ImportError
...
try:
    from google.generativeai.types import (
        ...
        # GenerationResponse здесь не импортируем
    )
    logger.info("Типы google.generativeai.types успешно импортированы (кроме GenerationResponse).")
    ...
    if hasattr(HarmBlockThreshold, 'BLOCK_NONE'):
        ...
        logger.warning("Атрибут 'BLOCK_NONE' не найден в HarmBlockThreshold."); all_enums_found = False
...
RETRY_ATTEMPTS = 5; RETRY_DELAY_SECONDS = 1
...
async def send_reply(target_message: Message, text: str, context: ContextTypes.DEFAULT_TYPE) -> Message | None:
    ...
    sent_message = None; chat_id = target_message.chat_id; message_id = target_message.message_id
    current_user_id = target_message.from_user.id if target_message.from_user else "Unknown"
    try:
        for i, chunk in enumerate(reply_chunks):
            if i == 0: sent_message = await context.bot.send_message(chat_id=chat_id, text=chunk, reply_to_message_id=message_id, parse_mode=ParseMode.MARKDOWN)
...
def _strip_thoughts_from_text(text_content: str | None) -> str:
    ...
    pattern = r"<мысли>.*?</мысли>\s*"; stripped_text = re.sub(pattern, "", text_content, flags=re.DOTALL | re.IGNORECASE)
    return stripped_text.strip()

# --- НОВАЯ ФУНКЦИЯ: Надежный парсер ответа Gemini ---
def _parse_gemini_response(
    response: Any, # Аннотация изменена на Any
    user_id: int | str, 
    chat_id: int | str, 
    attempt_num: int, 
    ...
    try:
        if not response or not hasattr(response, 'candidates') or not response.candidates:
            logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | ({context_str}) Ответ не содержит кандидатов или сам ответ None (попытка {attempt_num}).")
            pf_block_reason_str = "N/A"
            if response and hasattr(response, 'prompt_feedback') and response.prompt_feedback and hasattr(response.prompt_feedback, 'block_reason') and response.prompt_feedback.block_reason: # Добавлена проверка на существование block_reason
            ...
            else: parsed_text_or_error = f"🤖 ({context_str}) Модель не вернула структуру с кандидатами в ответе."
        ...
        if not hasattr(candidate, 'content') or not candidate.content or \
           not hasattr(candidate.content, 'parts') or not candidate.content.parts:
            ...
            # Используем getattr для безопасного доступа к FinishReason.<ENUM>.name
            if fr_str == getattr(FinishReason, 'SAFETY', 'SAFETY_DEFAULT_STRING_PLACEHOLDER_IF_MISSING').name if hasattr(FinishReason, 'SAFETY') else fr_str == 'SAFETY':
                parsed_text_or_error = f"🤖 ({context_str}) Ответ модели был отфильтрован из-за безопасности. ({sr_info})"
            elif fr_str == getattr(FinishReason, 'RECITATION', 'RECITATION_DEFAULT').name if hasattr(FinishReason, 'RECITATION') else fr_str == 'RECITATION':
                parsed_text_or_error = f"🤖 ({context_str}) Ответ модели был отфильтрован из-за цитирования защищенного контента."
            elif fr_str == getattr(FinishReason, 'OTHER', 'OTHER_DEFAULT').name if hasattr(FinishReason, 'OTHER') else fr_str == 'OTHER':
                parsed_text_or_error = f"🤖 ({context_str}) Модель завершила работу по неизвестной причине (OTHER)."
            elif fr_str == getattr(FinishReason, 'MAX_TOKENS', 'MAX_TOKENS_DEFAULT').name if hasattr(FinishReason, 'MAX_TOKENS') else fr_str == 'MAX_TOKENS':
                parsed_text_or_error = f"🤖 ({context_str}) Ответ модели был обрезан из-за достижения лимита токенов."
            elif fr_str == getattr(FinishReason, 'STOP', 'STOP_DEFAULT').name if hasattr(FinishReason, 'STOP') else fr_str == 'STOP': 
                 parsed_text_or_error = f"🤖 ({context_str}) Модель завершила работу, но не предоставила текстовый ответ (FR: {fr_str})."
            else: parsed_text_or_error = f"🤖 ({context_str}) Модель не предоставила текст (FR: {fr_str}, Safety: {sr_info})."
            return parsed_text_or_error
        
        try:
            if hasattr(response, 'text'): parsed_text_or_error = response.text 
            else:
                logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | ({context_str}) У объекта ответа отсутствует атрибут 'text'. FR: {fr_str}.")
                if candidate.content.parts:
                    text_parts_collected = [part.text for part in candidate.content.parts if hasattr(part, 'text') and part.text]
                    if text_parts_collected: parsed_text_or_error = "".join(text_parts_collected); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | ({context_str}) Текст успешно извлечен из 'parts'.")
                    else: parsed_text_or_error = f"🤖 ({context_str}) Модель не вернула текст в 'parts' (FR: {fr_str})."
                else: parsed_text_or_error = f"🤖 ({context_str}) Модель не вернула 'parts' в контенте (FR: {fr_str})."
        except ValueError: 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot_core_model_key = DEFAULT_MODEL; raw_bot_core_model_display_name = AVAILABLE_MODELS.get(bot_core_model_key, bot_core_model_key)
    current_model_key = get_user_setting(context, 'selected_model', DEFAULT_MODEL); raw_current_model_display_name = AVAILABLE_MODELS.get(current_model_key, current_model_key)
    ...
    author_channel_link_raw = "https://t.me/denisobovsyom"; date_knowledge_text_raw = "до начала 2025 года"
...
async def clear_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id; user = update.effective_user; user_id = user.id; first_name = user.first_name
    user_mention = f"{first_name}" if first_name else f"User {user_id}"
    context.chat_data['history'] = []; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | История чата очищена по команде от {user_mention}.")
...
async def set_temperature(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not context.args: await update.message.reply_text(f"🌡️ {user_mention}, твоя текущая температура (креативность): {current_temp:.1f}\nЧтобы изменить, напиши `/temp <значение>` (например, `/temp 0.8`)"); return
        ...
        set_user_setting(context, 'temperature', temp); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Температура установлена на {temp:.1f} для {user_mention}.")
        await update.message.reply_text(f"🌡️ Готово, {user_mention}! Твоя температура установлена на {temp:.1f}")
    except (ValueError, IndexError) as e: await update.message.reply_text(f"⚠️ Ошибка, {user_mention}. {e}. Укажи число от 0.0 до 2.0. Пример: `/temp 0.8`")
    except Exception as e: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | Ошибка в set_temperature: {e}", exc_info=True); await update.message.reply_text(f"❌ Ой, {user_mention}, что-то пошло не так при установке температуры.")

async def enable_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user; user_id = user.id; chat_id = update.effective_chat.id; first_name = user.first_name
    user_mention = f"{first_name}" if first_name else f"User {user_id}"
    set_user_setting(context, 'search_enabled', True); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Поиск включен для {user_mention}.")
    await update.message.reply_text(f"🔍 Поиск Google/DDG для тебя, {user_mention}, включён.")

async def disable_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user; user_id = user.id; chat_id = update.effective_chat.id; first_name = user.first_name
    user_mention = f"{first_name}" if first_name else f"User {user_id}"
    set_user_setting(context, 'search_enabled', False); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Поиск отключен для {user_mention}.")
...
async def enable_reasoning(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user; user_id = user.id; chat_id = update.effective_chat.id; first_name = user.first_name
    user_mention = f"{first_name}" if first_name else f"User {user_id}"
    set_user_setting(context, 'detailed_reasoning_enabled', True); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Режим углубленных рассуждений включен для {user_mention}.")
...
async def disable_reasoning(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user; user_id = user.id; chat_id = update.effective_chat.id; first_name = user.first_name
    user_mention = f"{first_name}" if first_name else f"User {user_id}"
    set_user_setting(context, 'detailed_reasoning_enabled', False); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Режим углубленных рассуждений отключен для {user_mention}.")
...
async def select_model_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_mention = f"{first_name}" if first_name else f"User {user_id}"; await query.answer(); callback_data = query.data
    if callback_data and callback_data.startswith("set_model_"):
        selected = callback_data.replace("set_model_", "")
        if selected in AVAILABLE_MODELS:
            set_user_setting(context, 'selected_model', selected); model_name = AVAILABLE_MODELS[selected]
            reply_text = f"Ок, {user_mention}, твоя модель установлена: **{model_name}**"; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Модель установлена на {model_name} для {user_mention}.")
            try: await query.edit_message_text(reply_text, parse_mode=ParseMode.MARKDOWN)
            except BadRequest as e_md:
                 if "Message is not modified" in str(e_md): logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Пользователь {user_mention} выбрал ту же модель: {model_name}"); await query.answer(f"Модель {model_name} уже выбрана.", show_alert=False)
                 else:
                     ...
                     except Exception as e_edit_plain: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | Не удалось изменить сообщение даже как простой текст для {user_mention}: {e_edit_plain}. Отправляю новое."); await context.bot.send_message(chat_id=chat_id, text=reply_text, parse_mode=ParseMode.MARKDOWN)
            except Exception as e: logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | Не удалось изменить сообщение (другая ошибка) для {user_mention}: {e}. Отправляю новое.", exc_info=True); await context.bot.send_message(chat_id=chat_id, text=reply_text, parse_mode=ParseMode.MARKDOWN)
        else:
...
async def perform_google_search(query: str, api_key: str, cse_id: str, num_results: int, session: aiohttp.ClientSession) -> list[str] | None:
    search_url = "https://www.googleapis.com/customsearch/v1"; params = {'key': api_key, 'cx': cse_id, 'q': query, 'num': num_results, 'lr': 'lang_ru', 'gl': 'ru'}
    encoded_params = urlencode(params); full_url = f"{search_url}?{encoded_params}"
    query_short = query[:50] + '...' if len(query) > 50 else query; logger.debug(f"Запрос к Google Search API для '{query_short}'...")
    try:
...
def extract_general_url(text: str) -> str | None:
    url_pattern = r'https?://[^\s/$.?#].[^\s]*'; match = re.search(url_pattern, text)
...
def get_current_time_str() -> str:
    try:
        ...
        months = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"]; month_name = months[now.month - 1]
    ...
    except Exception as e: logger.error(f"Ошибка получения времени для пояса {TARGET_TIMEZONE}: {e}"); now_utc = datetime.datetime.now(pytz.utc); return now_utc.strftime("%Y-%m-%d %H:%M:%S UTC")
...
async def reanalyze_image(update: Update, context: ContextTypes.DEFAULT_TYPE, file_id: str, user_question: str, original_user_id: int):
    ...
    for attempt in range(RETRY_ATTEMPTS):
        try:
            ...
            if reply_text is None: reply_text = f"❌ Ошибка при повторном анализе после {attempt + 1} попыток."; break
...
async def reanalyze_video(update: Update, context: ContextTypes.DEFAULT_TYPE, video_id: str, user_question: str, original_user_id: int):
    ...
    for attempt in range(RETRY_ATTEMPTS):
        try:
            ...
            if reply_text is None: reply_text = f"❌ Ошибка при ответе по видео после {attempt + 1} попыток."; break
...
async def _process_text_with_gemini(
    ...
    # image_file_id_for_history: str | None = None # Раскомментировать, если будет вызываться из handle_photo
    ):
    ...
    if not is_document_related:
        youtube_id = extract_youtube_id(user_text_for_prompt)
        if youtube_id:
            youtube_handled = True; first_name = original_update.effective_user.first_name
            user_mention = f"{first_name}" if first_name else f"User {user_id}"
            ...
            try: target_message_for_reply = await context.bot.get_chat(chat_id).get_message(original_message_id)
            except Exception as e_get_msg: logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | Не удалось получить оригинальное сообщение ({original_message_id}) для ответа: {e_get_msg}. Будет отправлено новое сообщение.")
            try: 
                reply_target = target_message_for_reply if target_message_for_reply else original_update.message
                if reply_target: await reply_target.reply_text(f"Окей, {user_mention}, сейчас гляну видео (ID: ...{youtube_id[-4:]}) и сделаю конспект...")
                else: await context.bot.send_message(chat_id, text=f"Окей, {user_mention}, сейчас гляну видео (ID: ...{youtube_id[-4:]}) и сделаю конспект...")
            ...
            prompt_for_summary = (f"(Текущая дата и время: {current_time_str})\nСделай краткий, но информативный конспект видео, находящегося ИСКЛЮЧИТЕЛЬНО по следующей ссылке. Не используй информацию из других источников или о других видео. Если видео по ссылке недоступно, сообщи об этом.\n**ССЫЛКА НА ВИДЕО ДЛЯ АНАЛИЗА:** {youtube_uri}\nОпиши основные пункты и ключевые моменты из СОДЕРЖИМОГО ИМЕННО ЭТОГО видео.")
            ...
            model_id_yt = get_user_setting(context, 'selected_model', DEFAULT_MODEL); temperature_yt = get_user_setting(context, 'temperature', 1.0)
            is_video_model = any(keyword in model_id_yt for keyword in VIDEO_CAPABLE_KEYWORDS)
            if not is_video_model:
                video_models = [m_id for m_id in AVAILABLE_MODELS if any(keyword in m_id for keyword in VIDEO_CAPABLE_KEYWORDS)]
                if video_models:
                    original_model_name_yt = AVAILABLE_MODELS.get(model_id_yt, model_id_yt); fallback_model_id_yt = next((m for m in video_models if 'flash' in m), video_models[0]); model_id_yt = fallback_model_id_yt
                ...
                else: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | (YouTubeSummary) Нет доступных video моделей."); await original_update.message.reply_text("❌ Нет доступных моделей для создания конспекта видео."); return
            logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (YouTubeSummary) Модель: {model_id_yt}, Темп: {temperature_yt}")
            reply_text_yt = None
            for attempt_yt in range(RETRY_ATTEMPTS):
                 try:
                     logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (YouTubeSummary) Попытка {attempt_yt + 1}/{RETRY_ATTEMPTS}...")
                     generation_config_yt = genai.GenerationConfig(temperature=temperature_yt, max_output_tokens=MAX_OUTPUT_TOKENS)
                     ...
                     response_obj_yt = await asyncio.to_thread(model_gemini_yt.generate_content, content_for_summary)
                 ...
                 except (BlockedPromptException, StopCandidateException) as e_block_stop_yt:
                      reason_str_yt = str(e_block_stop_yt.args[0]) if hasattr(e_block_stop_yt, 'args') and e_block_stop_yt.args else "неизвестна"
                      logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (YouTubeSummary) Конспект заблокирован/остановлен (попытка {attempt_yt + 1}): {e_block_stop_yt} (Причина: {reason_str_yt})"); reply_text_yt = f"❌ Не удалось создать конспект (ограничение модели)."; break
                 except Exception as e_yt:
                     error_message_yt = str(e_yt); logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (YouTubeSummary) Ошибка на попытке {attempt_yt + 1}: {error_message_yt[:200]}...")
                     is_retryable_yt = "500" in error_message_yt or "503" in error_message_yt
                     if "400" in error_message_yt or "429" in error_message_yt or "location is not supported" in error_message_yt or "unsupported language" in error_message_yt.lower(): reply_text_yt = f"❌ Ошибка при создании конспекта ({error_message_yt[:100]}...). Возможно, видео недоступно или на неподдерживаемом языке."; break
                     if is_retryable_yt and attempt_yt < RETRY_ATTEMPTS - 1: wait_time_yt = RETRY_DELAY_SECONDS * (2 ** attempt_yt); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (YouTubeSummary) Ожидание {wait_time_yt:.1f} сек..."); await asyncio.sleep(wait_time_yt); continue
                     else: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | (YouTubeSummary) Не удалось создать конспект после {attempt_yt + 1} попыток. Ошибка: {e_yt}", exc_info=True if not is_retryable_yt else False);
            ...
            history_entry_user_yt = { "role": "user", "parts": [{"text": user_message_with_id_for_history_and_prompt}], "youtube_video_id": youtube_id, "user_id": user_id, "message_id": original_message_id }; chat_history.append(history_entry_user_yt)
            ...
            if target_message_for_final_reply: await send_reply(target_message_for_final_reply, summary_for_user_display_yt, context)
            else: await context.bot.send_message(chat_id, text=summary_for_user_display_yt)
            while len(chat_history) > MAX_HISTORY_MESSAGES: chat_history.pop(0)
            return 
    
    await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    model_id_main = get_user_setting(context, 'selected_model', DEFAULT_MODEL); temperature_main = get_user_setting(context, 'temperature', 1.0)
    use_search_main = get_user_setting(context, 'search_enabled', True)
    search_context_snippets_main = []; search_provider_main = None; search_log_msg_main = "Поиск отключен пользователем"
    if use_search_main and not is_document_related:
        query_for_search_main = user_text_for_prompt
        query_short_main = query_for_search_main[:50] + '...' if len(query_for_search_main) > 50 else query_for_search_main
        search_log_msg_main = f"Поиск Google/DDG для '{query_short_main}'"; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | {search_log_msg_main}...")
        ...
        if google_results_main: search_provider_main = "Google"; search_context_snippets_main = google_results_main; search_log_msg_main += f" (Google: {len(search_context_snippets_main)} рез.)"
        else:
            search_log_msg_main += " (Google: 0 рез./ошибка)"; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Google не дал результатов. Пробуем DuckDuckGo...")
            try:
                ddgs = DDGS(); results_ddg_main = await asyncio.to_thread(ddgs.text, query_for_search_main, region='ru-ru', max_results=DDG_MAX_RESULTS)
                if results_ddg_main:
                    ddg_snippets_main = [r.get('body', '') for r in results_ddg_main if r.get('body')]
                    if ddg_snippets_main: search_provider_main = "DuckDuckGo"; search_context_snippets_main = ddg_snippets_main; search_log_msg_main += f" (DDG: {len(search_context_snippets_main)} рез.)"
                    else: search_log_msg_main += " (DDG: 0 текст. рез.)"
                else: search_log_msg_main += " (DDG: 0 рез.)"
            except Exception as e_ddg_main: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | Ошибка поиска DuckDuckGo: {e_ddg_main}", exc_info=True); search_log_msg_main += " (DDG: ошибка)"
    logger.info(f"UserID: {user_id}, ChatID: {chat_id} | {search_log_msg_main}")
    current_time_str_main = get_current_time_str(); time_context_str_main = f"(Текущая дата и время: {current_time_str_main})\n"
    ...
    if search_context_snippets_main:
        ...
        if search_context_lines_main:
            ...
            search_block_title_main = f"==== РЕЗУЛЬТАТЫ ПОИСКА ({search_provider_main}) ДЛЯ ОТВЕТА НА ВОПРОС ===="; search_block_instruction_main = f"Используй эту информацию для ответа на вопрос пользователя {USER_ID_PREFIX_FORMAT.format(user_id=user_id)}, особенно если он касается текущих событий или погоды."
            search_block_main = (f"\n\n{search_block_title_main}\n{search_context_text_main}\n===========================================================\n{search_block_instruction_main}"); final_prompt_parts_main.append(search_block_main)
    ...
    if get_user_setting(context, 'detailed_reasoning_enabled', True): final_prompt_parts_main.append(REASONING_PROMPT_ADDITION); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Добавлена инструкция для детального рассуждения.")
    final_gemini_prompt_text = "\n".join(final_prompt_parts_main); logger.debug(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Финальный промпт для Gemini (длина {len(final_gemini_prompt_text)}):\n{final_gemini_prompt_text[:600]}...")
    ...
    history_for_model_raw_main = []; current_total_chars_main = 0
    ...
    for entry_main in reversed(history_to_filter_main):
        entry_text_main = ""; entry_len_main = 0
        if entry_main.get("parts") and isinstance(entry_main["parts"], list) and len(entry_main["parts"]) > 0 and entry_main["parts"][0].get("text"):
            ...
            if "==== РЕЗУЛЬТАТЫ ПОИСКА" not in entry_text_main and "Важное указание по ссылке" not in entry_text_main: entry_len_main = len(entry_text_main)
        if current_total_chars_main + entry_len_main + len(final_gemini_prompt_text) <= MAX_CONTEXT_CHARS:
            if "==== РЕЗУЛЬТАТЫ ПОИСКА" not in entry_text_main and "Важное указание по ссылке" not in entry_text_main: history_for_model_raw_main.append(entry_main); current_total_chars_main += entry_len_main
        else: logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Обрезка истории по символам ({MAX_CONTEXT_CHARS}). Учтено {len(history_for_model_raw_main)} сообщ., ~{current_total_chars_main} симв."); break
    history_for_model_main = list(reversed(history_for_model_raw_main)); history_for_model_main.append({"role": "user", "parts": [{"text": final_gemini_prompt_text}]})
    ...
    for attempt_main in range(RETRY_ATTEMPTS):
        try:
            ...
            if reply_from_gemini_main and "🤖" not in reply_from_gemini_main and "❌" not in reply_from_gemini_main: logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Успешная генерация на попытке {attempt_main + 1}."); break 
            elif reply_from_gemini_main: logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Получено сообщение об ошибке/статусе от парсера: {reply_from_gemini_main}"); break
            ...
            logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Запрос заблокирован/остановлен моделью (попытка {attempt_main + 1}): {e_block_stop_main} (Причина: {reason_str_main})"); reply_from_gemini_main = f"❌ Запрос заблокирован/остановлен моделью."; break
        except Exception as e_main:
            error_message_main = str(e_main); logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Ошибка генерации на попытке {attempt_main + 1}: {error_message_main[:200]}...")
            ...
            if is_retryable_main and attempt_main < RETRY_ATTEMPTS - 1: wait_time_main = RETRY_DELAY_SECONDS * (2 ** attempt_main); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Ожидание {wait_time_main:.1f} сек..."); await asyncio.sleep(wait_time_main); continue
            else: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Не удалось выполнить генерацию после {attempt_main + 1} попыток. Ошибка: {e_main}", exc_info=True if not is_retryable_main else False);
            if reply_from_gemini_main is None: reply_from_gemini_main = f"❌ Ошибка при обращении к модели после {attempt_main + 1} попыток."; break # Добавлен break
    if reply_from_gemini_main is None: reply_from_gemini_main = "🤖 К сожалению, не удалось получить ответ от модели после нескольких попыток."; logger.error(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) reply_from_gemini_main остался None после всех попыток.")
    if not youtube_handled: chat_history.append({"role": "model", "parts": [{"text": reply_from_gemini_main}]})
    reply_to_send_user = reply_from_gemini_main
    if get_user_setting(context, 'detailed_reasoning_enabled', True) and reply_from_gemini_main and "🤖" not in reply_from_gemini_main and "❌" not in reply_from_gemini_main:
        cleaned_reply_main = _strip_thoughts_from_text(reply_from_gemini_main)
        if reply_from_gemini_main != cleaned_reply_main: logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Блок <мысли> удален из ответа.")
        reply_to_send_user = cleaned_reply_main
    target_message_for_final_reply_main = None
    try: target_message_for_final_reply_main = await context.bot.get_chat(chat_id).get_message(original_message_id)
    except Exception as e_get_msg_main: logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (MainProcess) Не удалось получить оригинальное сообщение ({original_message_id}) для ответа: {e_get_msg_main}. Используем update.message."); target_message_for_final_reply_main = original_update.message
    if target_message_for_final_reply_main: await send_reply(target_message_for_final_reply_main, reply_to_send_user, context)
    else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.effective_user: logger.warning(f"ChatID: {chat_id} | handle_message: Пропуск, нет effective_user."); return
    user_id = update.effective_user.id
    if not update.message or (not update.message.text and not update.message.photo and not update.message.document): logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | handle_message: Пропуск, нет сообщения или релевантного контента."); return

    original_message_id = update.message.message_id; user_text_content = ""
    if update.message.text: user_text_content = update.message.text.strip()
    if not user_text_content and not update.message.photo and not update.message.document: logger.info(f"UserID: {user_id}, ChatID: {chat_id} | handle_message: Пропуск, текст пуст и нет медиа для обработки."); return

    if update.message.reply_to_message and update.message.reply_to_message.text and user_text_content and not user_text_content.startswith('/'):
        replied_text = update.message.reply_to_message.text; user_question_for_reanalyze = user_text_content
        chat_history_reanalyze = context.chat_data.get("history", []); found_special_context_for_reanalyze = False
        try:
            for i in range(len(chat_history_reanalyze) - 1, -1, -1):
                ...
                if model_entry.get("role") == "model" and model_entry.get("parts") and isinstance(model_entry["parts"], list) and len(model_entry["parts"]) > 0:
                    ...
                    is_image_reply = model_text.startswith(IMAGE_DESCRIPTION_PREFIX) and replied_text.startswith(IMAGE_DESCRIPTION_PREFIX) and model_text[:150] == replied_text[:150]
                    is_video_reply = model_text.startswith(YOUTUBE_SUMMARY_PREFIX) and replied_text.startswith(YOUTUBE_SUMMARY_PREFIX) and model_text[:150] == replied_text[:150]
                        ...
                        if i > 0:
                            ...
                            if user_entry.get("role") == "user":
                                ...
                                if is_image_reply and "image_file_id" in user_entry:
                                    found_file_id = user_entry["image_file_id"]; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Ответ на описание фото. Запуск reanalyze_image для file_id: ...{found_file_id[-10:]}")
                                    await reanalyze_image(update, context, found_file_id, user_question_for_reanalyze, original_uploader_id); found_special_context_for_reanalyze = True; break
                                elif is_video_reply and "youtube_video_id" in user_entry:
                                    found_video_id = user_entry["youtube_video_id"]; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Ответ на конспект видео. Запуск reanalyze_video для video_id: {found_video_id}")
                                    await reanalyze_video(update, context, found_video_id, user_question_for_reanalyze, original_uploader_id); found_special_context_for_reanalyze = True; break
        except Exception as e_hist_search_reanalyze: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | Ошибка при поиске ID для reanalyze: {e_hist_search_reanalyze}", exc_info=True)
        if found_special_context_for_reanalyze: return
        if replied_text.startswith(IMAGE_DESCRIPTION_PREFIX) or replied_text.startswith(YOUTUBE_SUMMARY_PREFIX): logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | Ответ на спец. сообщение, но контекст для reanalyze не найден. Обработка как обычный текст.")

    if user_text_content: # Убедимся, что текст есть для вызова _process_text_with_gemini
        await _process_text_with_gemini(user_text_for_prompt=user_text_content, original_update=update, context=context, original_message_id=original_message_id, is_document_related=False)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_caption = message.caption if message.caption else ""; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Обработка фото как изображения (Vision).")
    await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    ...
    parts_vision = [{"text": prompt_text_vision}, {"inline_data": {"mime_type": mime_type, "data": b64_data}}]; content_for_vision = [{"role": "user", "parts": parts_vision}]
    ...
    for attempt_vision in range(RETRY_ATTEMPTS):
        try:
            ...
            if reply_text_vision and "🤖" not in reply_text_vision and "❌" not in reply_text_vision: logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (Vision) Успешный анализ на попытке {attempt_vision + 1}."); break
        ...
        except (BlockedPromptException, StopCandidateException) as e_block_stop_vision:
             ...
             logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (Vision) Анализ заблокирован/остановлен (попытка {attempt_vision + 1}): {e_block_stop_vision} (Причина: {reason_str_vision})"); reply_text_vision = f"❌ Анализ изображения заблокирован/остановлен моделью."; break
        except Exception as e_vision:
             error_message_vision = str(e_vision); logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | (Vision) Ошибка на попытке {attempt_vision + 1}: {error_message_vision[:200]}...")
             is_retryable_vision = "500" in error_message_vision or "503" in error_message_vision
             if "400" in error_message_vision or "429" in error_message_vision or "location is not supported" in error_message_vision: reply_text_vision = f"❌ Ошибка при анализе изображения ({error_message_vision[:100]}...)."; break
             if is_retryable_vision and attempt_vision < RETRY_ATTEMPTS - 1: wait_time_vision = RETRY_DELAY_SECONDS * (2 ** attempt_vision); logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (Vision) Ожидание {wait_time_vision:.1f} сек..."); await asyncio.sleep(wait_time_vision); continue
             else: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | (Vision) Не удалось выполнить анализ после {attempt_vision + 1} попыток. Ошибка: {e_vision}", exc_info=True if not is_retryable_vision else False);
             if reply_text_vision is None: reply_text_vision = f"❌ Ошибка при анализе изображения после {attempt_vision + 1} попыток."; break # Добавлен break
    ...
    chat_history = context.chat_data.setdefault("history", []); user_text_for_history_vision = USER_ID_PREFIX_FORMAT.format(user_id=user_id) + (user_caption if user_caption else "Пользователь прислал фото.")
    history_entry_user_photo = { "role": "user", "parts": [{"text": user_text_for_history_vision}], "image_file_id": photo_file_id, "user_id": user_id, "message_id": user_message_id }; chat_history.append(history_entry_user_photo)
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    allowed_mime_types = ('application/octet-stream',); mime_type = doc.mime_type or "application/octet-stream"
    is_allowed_prefix = any(mime_type.startswith(prefix) for prefix in allowed_mime_prefixes); is_allowed_type = mime_type in allowed_mime_types
    ...
    await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    text_from_doc = None; detected_encoding = None; encodings_to_try = ['utf-8-sig', 'utf-8', 'cp1251', 'latin-1', 'cp866', 'iso-8859-5']
    ...
    for encoding in unique_encodings:
        try: text_from_doc = file_bytes.decode(encoding); detected_encoding = encoding; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | Файл '{doc.file_name}' успешно декодирован как {encoding}."); break
    ...
    if text_from_doc is None: logger.error(f"UserID: {user_id}, ChatID: {chat_id} | Не удалось декодировать '{doc.file_name}' ни одной из кодировок: {unique_encodings}"); await update.message.reply_text(f"❌ Не удалось прочитать файл `{doc.file_name}`. Попробуйте UTF-8.", parse_mode=ParseMode.MARKDOWN); return
    if not text_from_doc.strip() and len(file_bytes) > 0: logger.warning(f"UserID: {user_id}, ChatID: {chat_id} | Файл '{doc.file_name}' дал пустой текст после декодирования ({detected_encoding})."); await update.message.reply_text(f"⚠️ Не удалось извлечь текст из файла `{doc.file_name}`.", parse_mode=ParseMode.MARKDOWN); return
    ...
    if len(text_from_doc) > MAX_FILE_CHARS:
        truncated_text = text_from_doc[:MAX_FILE_CHARS]; last_newline = truncated_text.rfind('\n')
    ...
    user_caption_doc = message.caption if message.caption else ""; file_name_for_prompt_doc = doc.file_name or "файл"
    ...
    if user_caption_doc: escaped_caption_doc = user_caption_doc.replace('"', '\\"'); user_prompt_for_gemini_doc = f"Пользователь загрузил файл `{file_name_for_prompt_doc}` с комментарием: \"{escaped_caption_doc}\". {file_context_for_prompt_doc}\nПроанализируй, пожалуйста."
    else: user_prompt_for_gemini_doc = f"Пользователь загрузил файл `{file_name_for_prompt_doc}`. {file_context_for_prompt_doc}\nЧто можешь сказать об этом тексте?"
    if get_user_setting(context, 'detailed_reasoning_enabled', True): user_prompt_for_gemini_doc += REASONING_PROMPT_ADDITION; logger.info(f"UserID: {user_id}, ChatID: {chat_id} | (Document) Добавлена инструкция для детального рассуждения.")
    logger.debug(f"UserID: {user_id}, ChatID: {chat_id} | Передача управления в _process_text_with_gemini с текстом документа.")
    await _process_text_with_gemini(user_text_for_prompt=user_prompt_for_gemini_doc, original_update=update, context=context, original_message_id=message.message_id, is_document_related=True)
# ====================================================================

async def setup_bot_and_server(stop_event: asyncio.Event):
    application = Application.builder().token(TELEGRAM_BOT_TOKEN).build()
    timeout = aiohttp.ClientTimeout(total=60.0, connect=10.0, sock_connect=10.0, sock_read=30.0); aiohttp_session = aiohttp.ClientSession(timeout=timeout)
    application.bot_data['aiohttp_session'] = aiohttp_session; logger.info("Сессия aiohttp создана и сохранена в bot_data.")
    ...
    application.add_handler(CommandHandler("temp", set_temperature)); application.add_handler(CommandHandler("search_on", enable_search)); application.add_handler(CommandHandler("search_off", disable_search))
    application.add_handler(CommandHandler("reasoning_on", enable_reasoning)); application.add_handler(CommandHandler("reasoning_off", disable_reasoning))
    application.add_handler(CallbackQueryHandler(select_model_callback, pattern="^set_model_"))
    application.add_handler(MessageHandler(filters.PHOTO, handle_photo)); application.add_handler(MessageHandler(filters.Document.ALL, handle_document)); application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
    try:
        await application.initialize()
        commands = [BotCommand("start", "Начать работу и инфо"), BotCommand("model", "Выбрать модель Gemini"), BotCommand("temp", "Установить температуру (креативность)"), BotCommand("search_on", "Включить поиск Google/DDG"), BotCommand("search_off", "Выключить поиск Google/DDG"), BotCommand("reasoning_on", "Вкл. углубленные рассуждения"), BotCommand("reasoning_off", "Выкл. углубленные рассуждения"), BotCommand("clear", "Очистить историю чата"),]
        await application.bot.set_my_commands(commands); logger.info("Команды меню бота успешно установлены.")
        webhook_host_cleaned = WEBHOOK_HOST.rstrip('/'); webhook_path_segment = GEMINI_WEBHOOK_PATH.strip('/'); webhook_url = f"{webhook_host_cleaned}/{webhook_path_segment}"
        logger.info(f"Попытка установки вебхука: {webhook_url}"); secret_token = os.getenv('WEBHOOK_SECRET_TOKEN')
        ...
        logger.info(f"Вебхук успешно установлен на {webhook_url}" + (" с секретным токеном." if secret_token else ".")); web_server_coro = run_web_server(application, stop_event)
...
async def run_web_server(application: Application, stop_event: asyncio.Event):
    ...
    app.router.add_get('/', health_check); app['bot_app'] = application
    webhook_path = GEMINI_WEBHOOK_PATH.strip('/'); 
    if not webhook_path.startswith('/'): webhook_path = '/' + webhook_path
    app.router.add_post(webhook_path, handle_telegram_webhook); logger.info(f"Вебхук будет слушаться на пути: {webhook_path}")
...
async def handle_telegram_webhook(request: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    try:
        ...
        logger.debug(f"Получен Update ID: {update.update_id} от Telegram."); await application.process_update(update)
...
async def main():
    ...
    logging.getLogger('aiohttp.access').setLevel(logging.WARNING); logging.getLogger('google.generativeai').setLevel(logging.INFO); logging.getLogger('duckduckgo_search').setLevel(logging.INFO)
